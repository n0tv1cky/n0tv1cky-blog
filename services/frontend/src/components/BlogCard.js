...
export default function BlogCard({ blog }) {
    return (
        <Link href={`/blogs/${blog.slug}`}>
            <article className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-6 hover:shadow-md transition-all duration-200 group cursor-pointer">
                <div className="flex justify-between items-start gap-4">
                    <div className="flex-1">
                        ...
                        {blog.description && (
                            <p className="text-gray-600 dark:text-gray-400 mb-4 leading-relaxed">{blog.description}</p>
                        )}
                        {blog.reading_time && (
